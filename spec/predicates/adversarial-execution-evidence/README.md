...
| [`settled-readings.md`](settled-readings.md) | The reading this text takes wherever a second implementation, reading before it ran, found the text admitted two. Nine places: five in the encoding profile, three in the `result` recompute and the seal's observed set, and the pinned `networkPosture` digest, which the registry page now defines because the two readings there accept and refuse disjoint sets of substrate-row statements. |
...
`settled-readings.md` is that position applied to itself. Each entry there
names a reading that two implementations happened to agree on, or that one
implementation chose and recorded; agreement is evidence the text is
determinate and not proof of it, so each entry is also a vector owed.
